...
import { OrderingMistakes, OrderingSpread, OrderingSubmissions, SequenceReadout } from './OrderingResults'
      ...
      <>
        <section className="panel result-panel ordering-results-panel">
          <div className="panel-heading">
            ...
            <span className="hotspot-heading-actions">
              {window.interactDesktop?.openOrderingReview && (
                <button
                  aria-label="放大檢視作答"
                  className="icon-button"
                  title="放大檢視作答"
                  type="button"
                  onClick={() => void window.interactDesktop?.openOrderingReview(question.session_id, question.id)}
                >
                  <Maximize2 size={20} />
                </button>
              )}
